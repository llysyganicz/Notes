...
- `Models/` — domain records (`AppSettings`, `NoteTreeNode`).
- `Services/` — interfaces + Avalonia/IO-bound implementations (settings, scanner, tree builder, deleter, folder picker, confirm dialog).
- `ViewModels/` — CommunityToolkit.Mvvm view models.
- `Views/` — custom dialogs (e.g. `ConfirmDialog`).
- `Notes.Tests/` — xUnit test project; pure-logic services only.
- `App.axaml(.cs)`, `Program.cs`, `MainWindow.*` — application root, DI composition root, shell window.
- `Notes.slnx` — solution file (XML format produced by .NET 10's `dotnet new sln`; `dotnet build` / `dotnet test` work without specifying the file).

## Build & Run

```sh
dotnet build       # builds Notes + Notes.Tests
dotnet run         # launches the desktop app
...
- **Dependency injection — Microsoft.Extensions.DependencyInjection:** services are registered in `Program.cs` and resolved through the static `App.Services` provider. Services are singletons; ViewModels and Windows are transients.
- **Compiled bindings** are enabled by default — use `x:DataType` in AXAML and avoid reflection-based bindings.
- **Share behavior across ViewModels via DI-injected services, not base-class hierarchies.** When two ViewModels need the same logic, register it as a service in `Program.cs` and inject it into both rather than introducing a `ViewModelBase` parent.
