...
- **Dependency injection — Microsoft.Extensions.DependencyInjection:** services from both `Notes.Core` and `Notes` are registered in `Notes/Program.cs` (the single composition root) and resolved through the static `App.Services` provider. Services are singletons; ViewModels and Windows are transients.
- **`Notes.Core` stays platform-agnostic:** the `Notes.Core` project must not reference Avalonia or any UI framework — it holds models, pure-logic services, and headless ViewModels. Anything that touches Avalonia primitives (dialogs, pickers, dispatcher-bound schedulers, UI-coupled ViewModels) lives in `Notes`.
...
- **File system in tests — `MockFileSystem`:** inject `IFileSystem` (from `System.IO.Abstractions`) into services that touch disk; use `new MockFileSystem()` (from `System.IO.Abstractions.TestingHelpers`) in tests. Never access the real file system from either test project (`Notes.Core.Tests/`, `Notes.Tests/`).
...
The solution is split into a platform-agnostic core (`Notes.Core`) and the Avalonia UI layer (`Notes`), each with its own test project. The dependency direction is one-way: `Notes` → `Notes.Core` (never the reverse).

- `Notes.slnx` — solution file at the repo root (XML format produced by .NET 10's `dotnet new sln`; `dotnet build` / `dotnet test` work without specifying the file). Lists all four projects below.
- `Notes.Core/` — platform-agnostic library (no Avalonia reference): models, pure-logic services, and headless ViewModels.
  - `Notes.Core/Models/` — domain records: `AppSettings`, `NoteTreeNode`, `NoteMetadata`, `NoteSearchResult`, `EditorPaneState`, `FormDefinition`, `FormField`, `TemplateInfo`.
  - `Notes.Core/Services/` — interfaces (`I*.cs`) + implementations, grouped by concern:
    - *Workspace & files:* `SettingsService`, `WorkspaceScanner`, `NoteTreeBuilder`, `NoteFileService`, `NoteFolderService`, `NoteDeleter`, `OrphanedTempCleaner`, `NameValidator`, `PathGuard` (with `PathContainmentException`)
    - *Search:* `NoteSearchIndex`, `NoteMetadataParser`
    - *Templates:* `TemplateCatalog`, `TemplateParser`, `TemplateRenderer`
  - `Notes.Core/ViewModels/` — UI-independent view models: `TemplateFormViewModel`; `Fields/` subdirectory holds per-field-type VMs (`FieldVm`, `TextFieldVm`, `DateFieldVm`, `NumberFieldVm`, `SelectFieldVm`).
  - `Notes.Core/Messaging/` — `Messages.cs` — WeakReferenceMessenger message types used for cross-VM communication.
- `Notes/` — Avalonia desktop app; references `Notes.Core`.
  - `Notes/Services/` — Avalonia-coupled services (interfaces + implementations):
    - *Editor:* `AutoSaveScheduler`
    - *UI dialogs & pickers:* `ConfirmDialogService`, `NewNoteDialogService`, `TemplatePickerDialogService`, `TemplateFormDialogService`, `AvaloniaFolderPicker`
  - `Notes/ViewModels/` — CommunityToolkit.Mvvm view models bound to the UI: `MainWindowViewModel`, `NoteTreeViewModel`, `NoteEditorViewModel`, `NoteSearchViewModel`, `TemplatePickerViewModel`, `ViewModelLocator`.
...
- `Notes.Core.Tests/` — xUnit (v3) test project for `Notes.Core`; pure-logic services and headless view models.
  - `Notes.Core.Tests/Fakes/` — in-process fakes such as `ThrowingFileSystem`.
- `Notes.Tests/` — xUnit (v3) test project for `Notes`; Avalonia-coupled view models (runs headless via `Avalonia.Headless.XUnit`).
...
dotnet test                    # runs both xUnit suites (Notes.Core.Tests + Notes.Tests)
```
