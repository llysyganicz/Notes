...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "workflow friction — existing tools slow the user down or are abandoned"
    - topic: "insight"
      decision: "combine plain-text editor with syntax highlighting (Notable) and template system with YAML frontmatter fields (Obsidian + plugins) in one lightweight app"
    - topic: "primary persona scope"
      decision: "developers / technical users who write markdown daily"
  frs_drafted: 10
...
### MVP flow (locked)
1. User opens the app → sees a list of existing markdown notes (or empty state)
2. User clicks "New Note" → a blank markdown editor opens with syntax highlighting
3. User writes markdown content, adds tags in YAML frontmatter
4. User saves → the note is written as a `.md` file to disk
5. User switches to preview → sees the note rendered as HTML
6. User returns to the list → searches by name or tag to find a note
7. User opens the found note → edits it
8. User clicks "New from Template" → picks a template
9. A form appears with fields defined in the template's YAML frontmatter
10. User fills the form → a note is created with placeholders replaced by the filled values
...
### Templates
- FR-008: User can create a template with field definitions in YAML frontmatter and placeholders in the body. Priority: must-have
...
- FR-009: User can create a new note from a template, filling a form generated from the template's field definitions. Priority: must-have
  > Socrates: No counter-argument; stands as written.

### Workspace
- FR-010: User can select a working directory (notes folder) on first launch. Priority: must-have
...
The template file is a standard `.md` file whose YAML frontmatter declares field names (and optionally types/defaults) and whose body contains placeholders (e.g. `{{field_name}}`). When the user creates a note from a template, the app parses the frontmatter to generate a form, collects the user’s responses, and writes a new `.md` file with frontmatter populated and all placeholders replaced by the corresponding values. The resulting file is a plain markdown note indistinguishable from one created manually.
...
- Timeline-cost ack: present (3 weeks, within target)
- Non-Goals: present (1 entry)
