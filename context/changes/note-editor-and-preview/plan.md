...
- [x] 3.1 `dotnet build` succeeds with no new warnings — c7bfbcf
- [x] 3.2 `dotnet test` remains green; new editor VM tests pass — c7bfbcf

#### Manual

- [x] 3.3 `View → Preview` (or Ctrl+E) on an open note replaces the editor with a rendered preview reflecting current edits — c7bfbcf
- [x] 3.4 Toggling back returns to the editor with document text intact — c7bfbcf
- [x] 3.5 Switching notes while in preview opens the new note in edit mode (preview state resets) — c7bfbcf
- [x] 3.6 With no note selected, View → Preview and Ctrl+E are no-ops (no error, no visible change) — c7bfbcf
- [x] 3.7 Mixed markdown (headings, lists, fenced code, links, image, table) renders with appropriate styling — c7bfbcf
- [x] 3.8 Editing is not possible while preview is showing (no editor visible) — c7bfbcf
