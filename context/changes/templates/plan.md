...
- [ ] 1.2 Unit tests pass: `dotnet test`
- [ ] 1.3 Parser: ordered definition; missing/malformed/absent → empty; field order = document order; entries + format captured
- [ ] 1.4 Renderer: body substitution; undeclared token verbatim; blank → empty; `form` removed; other keys verbatim; `form`-only → no fence; frontmatter tokens untouched
