...
- [ ] 2.2 On a test PR, the `build-test-format` check runs and passes against the green tree
- [ ] 2.3 A PR with a deliberate format violation makes the check fail (red)

#### Manual

- [ ] 2.4 Reported check name is exactly `build-test-format` (recorded before binding)
- [ ] 2.5 Workflow run time is acceptable
