...
- [ ] 2.2 New durable-write + sweep tests pass: `dotnet test`
- [ ] 2.3 Fault test fails if `Save` reverted to plain `WriteAllText` (spike + restore)
- [ ] 2.4 Sweep test confirms `*.md` notes never deleted and sweep stays under the root
- [ ] 2.5 Existing round-trip + overwrite tests still pass

#### Manual

- [ ] 2.6 App saves/reloads correctly end-to-end; no stray `.tmp` files
- [ ] 2.7 Hand-made `orphan.md.tmp` removed on next app start; real notes intact
- [ ] 2.8 No perceptible regression saving a large note
